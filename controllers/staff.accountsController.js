...
exports.addStaffAccount = async (req, res) => {
  try {
    ...
    const existingStaffAccount = await StaffAccount.findOne({ staffName: staffName });
    ...
    console.error(error);
    res.status(500).json({ error: 'An error occurred while creating the staff account' });
  }
};
...
exports.transferAmount = async (req, res) => {
    try {
        ...
        const receiverAccount = await StaffAccount.findOne({ staffCode: receiverId });
        if (!receiverAccount) {
        ...
        console.error(error);
        res.status(500).json({ error: 'An error occurred while transferring amount' });
    }
};
      ...
      staffAccount.accountHistory.forEach(transaction => {
          if (transaction.type === 'Received') {
              if (transaction.paymentMethod === 'Cash') {
                  ...
                  const bankName = transaction.paymentMethod.split('(')[1].split(')')[0];
