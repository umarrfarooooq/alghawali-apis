...
exports.updateMaidAvailablity = async (req, res) => {
  try {
    ...
    // Toggle the isHired status
    existingMaid.isHired = !existingMaid.isHired;

    const updatedMaid = await existingMaid.save();

    if (!updatedMaid) {
      return res.status(500).json({ error: 'Failed to update Maid status' });
    }
