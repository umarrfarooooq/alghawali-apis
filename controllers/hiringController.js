...
exports.returnMaid = async (req, res) => {
  ...
  try {
    ...
    if (!isValidObjectId(sendedBy) || !isValidObjectId(staffAccount)) {
      await session.abortTransaction();
      return res
        .status(400)
        .json({ error: "Invalid sendedBy or staffAccount ID" });
    }

    const [senderStaff, actionStaff] = await Promise.all([
      StaffAccount.findById(sendedBy).session(session),
    ...
    const transaction = new Transaction({
      ...
      status:
        req.staffRoles && req.staffRoles.includes(roles.fullAccessOnAccounts)
          ? "Approved"
          : "Pending",
      description: `Maid return: ${returnReason}`,
    });
    ...
    customerAccount.receivedAmount -= totalReturnAmount;
    officeCharges > 0
      ? (customerAccount.officeCharges += parseFloat(officeCharges))
      : 0;
    customerAccount.transactions.push(transaction._id);

    const staffAccountDoc = await StaffAccount.findById(sendedBy).session(
      session
    );
    if (!staffAccountDoc) {
      await session.abortTransaction();
    ...
    if (req.staffRoles && req.staffRoles.includes(roles.fullAccessOnAccounts)) {
      staffAccountDoc.balance -= parseFloat(returnAmount);
      staffAccountDoc.totalSentAmount += parseFloat(returnAmount);
      customerAccount.returnAmount += parseFloat(returnAmount);
    } else {
      staffAccountDoc.pendingSentAmount += parseFloat(returnAmount);
    ...
    if (totalRefunded >= customerAccount.receivedAmount) {
    ...
    staffAccountDoc.transactions.push(transaction._id);

    await Promise.all([
      maid.save({ session }),
      customerAccount.save({ session }),
      staffAccountDoc.save({ session }),
    ]);
...
exports.replaceMaid = async (req, res) => {
  ...
  try {
    ...
    if (priceDifference > 0) {
      ...
      transactionDescription = `Maid replacement: ${replaceReason}. Additional payment required.`;
      ...
      transactionDescription = `Maid replacement: ${replaceReason}. Refund due to price difference.`;
...
exports.updateCustomerPayment = async (req, res) => {
  ...
  try {
    ...
    if (!paymentReason || typeof paymentReason !== "string") {
...
exports.handlePendingTransaction = async (req, res) => {
  ...
  try {
    ...
    if (action === "approve") {
      ...
      const totalReturned =
        customerAccount.returnAmount + customerAccount.pendingReturnAmount;
      const netAmount = totalReceived - totalReturned;

      if (netAmount >= customerAccount.totalAmount) {
        customerAccount.cosPaymentStatus = "Fully Paid";
      } else if (netAmount > 0) {
        if (totalReturned > 0) {
          customerAccount.cosPaymentStatus = "Partially Refunded";
